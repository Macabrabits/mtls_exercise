...
```bash
openssl req -new -x509 -sha256 -days 365 -key ca-key.pem -out ca.crt-out ca.crt
```

### Optional Stage: View Certificate's Content
```bash
openssl x509 -in ca.crt -text
openssl x509 -in ca.crt -purpose -noout -text
```
...
COMMAND | CONVERSION
---|---
`openssl x509 -outform der -in cert.crt -out cert.der` | PEM to DER
`openssl x509 -inform der -in cert.der -out cert.crt` | DER to PEM
`openssl pkcs12 -in cert.pfx -out cert.crt -nodes` | PFX to PEM

## Verify Certificates
`openssl verify -CAfile ca.crt -verbose cert.crt`

## Install the CA Cert as a trusted root CA

### On Debian & Derivatives
- Move the CA certificate (`ca.crt`) into `/usr/local/share/ca-certificates/ca.crt`.
...
### On Fedora
- Move the CA certificate (`ca.crt`) to `/etc/pki/ca-trust/source/anchors/ca.crt` or `/usr/share/pki/ca-trust-source/anchors/ca.crt`
...
Assuming the path to your generated CA certificate as `C:\ca.crt`, run:
```powershell
...
```sh
certutil.exe -addstore root C:\ca.crt
```
...
5. Locate the certificate file `ca.crt` on your SD Card/Internal Storage using the file manager.
6. Select to load it.
7. Done!